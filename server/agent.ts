...
import { DynamicTool } from "@langchain/core/tools"              // For creating custom tools/functions
  ...
  try {
    ...
    const GraphState = Annotation.Root({
      messages: Annotation<BaseMessage[]>({
        ...
        reducer: (x: any, y: any) => x.concat(y), // Simply concatenate old messages (x) with new messages (y)
      }),
    })

    // Create a custom tool for searching furniture inventory using DynamicTool to avoid type issues
